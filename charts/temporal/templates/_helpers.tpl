...
{{- $root := index . 0 -}}
{{- $store := index . 1 -}}
{{- $storeConfig := index $root.Values.server.config.persistence $store -}}
{{- if hasKey $storeConfig "driver" -}}
{{- $storeConfig.driver -}}
{{- else if hasKey $storeConfig "sql" -}}
{{- print "sql" -}}
{{- else if hasKey $storeConfig "cassandra" -}}
{{- print "cassandra" -}}
{{- else if hasKey $storeConfig "elasticsearch" -}}
{{- print "elasticsearch" -}}
{{- else -}}
{{- print "custom" -}}
{{- end -}}
...
{{- $store := index . 1 -}}
{{- $storeConfig := index $root.Values.server.config.persistence $store -}}
{{- $driverConfig := index $storeConfig "sql" -}}
...
{{- $store := index . 1 -}}
{{- $storeConfig := index $root.Values.server.config.persistence $store -}}
{{- $driverConfig := index $storeConfig "cassandra" -}}
{{- if $driverConfig.hosts -}}
{{- if kindIs "slice" $driverConfig.hosts -}}
{{- join "," $driverConfig.hosts -}}
{{- else -}}
{{- $driverConfig.hosts -}}
{{- end -}}
{{- else -}}
